...
foreach ($form in $normalizationForms) {
    ...
    # Apply normalization using .NET string.Normalize method
    try {
        ...
        $normalizedFiles += $outputFile
        Write-Host "  Using original content as fallback"
    }
}
...
foreach ($file in $normalizedFiles) {
    ...
    $form = $file -replace "normalize_" -replace ".css"
        ...
        try {
            ...
            foreach ($form in $normalizationForms) {
                try {
                    ...
                    Write-Host "  $form`: Error in normalization - $($_.Exception.Message)"
                }
            }
        }
        catch {
            Write-Host "  Error reading $variant - $($_.Exception.Message)"
        }
    }
}
