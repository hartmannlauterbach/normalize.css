...
Write-Host "`n=== ENCODING + NORMALIZATION MATRIX ANALYSIS ===" -ForegroundColor Cyan

foreach ($encoding in $encodingVariants) {
    Write-Host "`n--- $encoding Encoding + Normalization ---" -ForegroundColor Yellow
    ...
    if (Test-Path $encodedFile) {
        try {
            ...
            if ($encoding -eq "utf8") {
                $content = Get-Content $encodedFile -Raw
            } elseif ($encoding -eq "utf16be") {
            ...
            Write-Host "Decoded length: $($content.Length)" -ForegroundColor Gray
            
            # Apply all normalization forms
            foreach ($form in $normalizationForms) {
                try {
                    $normalized = [System.Text.Normalization]::NormalizeString($content, [System.Text.NormalizationForm]::$form)
                    $outputFile = "normalize_${encoding}_${form}_detailed.css"
                    $normalized | Out-File -FilePath $outputFile -Encoding UTF8
                    
                    $asteriskCount = ($normalized.ToCharArray() | Where-Object { $_ -eq '*' }).Count
                    Write-Host "  $form: $asteriskCount asterisks" -ForegroundColor Gray
                    
                    # Check for content changes
                    if ($normalized -ne $content) {
                        Write-Host "  $form: CONTENT CHANGED!" -ForegroundColor Red
                        
                        # Check asterisk preservation
                        $origAsterisks = ($content.ToCharArray() | Where-Object { $_ -eq '*' }).Count
                        $normAsterisks = ($normalized.ToCharArray() | Where-Object { $_ -eq '*' }).Count
                        
                        if ($origAsterisks -ne $normAsterisks) {
                            Write-Host "  $form: ASTERISKS CHANGED! $origAsterisks → $normAsterisks" -ForegroundColor Red
                            Write-Host "  $form: POTENTIAL EXPLOITATION VECTOR!" -ForegroundColor Red
                        }
                    }
                }
                catch {
                    Write-Host "  $form: Normalization failed" -ForegroundColor Red
                }
            }
        }
        catch {
...
# Check 4: Parser confusion attacks
Write-Host "`n4. PARSER CONFUSION ANALYSIS" -ForegroundColor Yellow

# Test if normalized files can be parsed differently
foreach ($form in $normalizationForms) {
    $normFile = "normalize_${form}_detailed.css"
    if (Test-Path $normFile) {
        $normContent = Get-Content $normFile -Raw
        ...
        # Test CSS parsing
        $origCSSRules = [regex]::Matches($originalContent, '[^{]*\{[^}]*\}')
        $normCSSRules = [regex]::Matches($normContent, '[^{]*\{[^}]*\}')
        
        if ($normCSSRules.Count -ne $origCSSRules.Count) {
            Write-Host "CRITICAL: $form CSS parsing changed! $($origCSSRules.Count) → $($normCSSRules.Count)" -ForegroundColor Red
            $exploitationVectors += @{
                Type = "ParserConfusion"
                Normalization = $form
                Parser = "CSS"
                OriginalRules = $origCSSRules.Count
                NewRules = $normCSSRules.Count
                Severity = "High"
            }
        }
        
        # Test if it can be parsed as JavaScript
        try {
            $null = [System.Management.Automation.PSParser]::Tokenize($normContent) 2>$null
            # If we get here, it might be parseable as PowerShell
            Write-Host "WARNING: $form might be parseable as PowerShell" -ForegroundColor Yellow
            $exploitationVectors += @{
                Type = "PolyglotPotential"
                Normalization = $form
                Parser = "PowerShell"
                Severity = "Medium"
            }
